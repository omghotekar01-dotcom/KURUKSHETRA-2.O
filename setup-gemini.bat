...
echo.
echo ============================================================
echo   GEMINI FREE-TIER SETUP - AI AGENTIC BUG ROUTER
echo ============================================================
echo.
...
echo [GEMINI] This uses the Gemini Developer API free tier when your Google project is eligible.
echo [GEMINI] Your key is stored only in local .env, which is gitignored.
echo [GEMINI] Recommended default: gemini-3.5-flash-lite for fast, higher-headroom fallback.
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop'; $envPath=Join-Path (Get-Location) '.env'; $secure=Read-Host '[GEMINI] Paste Gemini API key (input hidden)' -AsSecureString; $ptr=[Runtime.InteropServices.Marshal]::SecureStringToBSTR($secure); try { $key=[Runtime.InteropServices.Marshal]::PtrToStringBSTR($ptr) } finally { [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($ptr) }; if([string]::IsNullOrWhiteSpace($key)){ throw 'No key entered.' }; $content=Get-Content $envPath -Raw; function Set-Line([string]$name,[string]$value){ $script:content=[regex]::Replace($script:content,'(?m)^'+[regex]::Escape($name)+'=.*$', $name+'='+$value); if($script:content -notmatch '(?m)^'+[regex]::Escape($name)+'='){ $script:content=$script:content.TrimEnd()+[Environment]::NewLine+$name+'='+$value+[Environment]::NewLine } }; Set-Line 'LLM_PROVIDER' 'auto'; Set-Line 'GEMINI_API_KEY' $key; Set-Line 'GEMINI_BASE_URL' 'https://generativelanguage.googleapis.com/v1beta/openai'; Set-Line 'GEMINI_MODEL' 'gemini-3.5-flash-lite'; Set-Content -Path $envPath -Value $content -Encoding UTF8; $headers=@{Authorization='Bearer '+$key}; $body=@{model='gemini-3.5-flash-lite';messages=@(@{role='user';content='Reply with exactly READY and nothing else.'});temperature=0} | ConvertTo-Json -Depth 8; $r=Invoke-RestMethod -Method Post -Uri 'https://generativelanguage.googleapis.com/v1beta/openai/chat/completions' -Headers $headers -ContentType 'application/json' -Body $body -TimeoutSec 30; $reply=[string]$r.choices[0].message.content; if([string]::IsNullOrWhiteSpace($reply)){ throw 'Gemini returned an empty response.' }; Write-Host ('[GEMINI] Live API reply: '+$reply.Trim()); Write-Host '[GEMINI] VERIFIED: free-tier Gemini fallback is configured.'"
...
echo.
echo [GEMINI] READY.
echo [GEMINI] Provider policy: local Qwen first, Gemini fallback second, deterministic evidence fallback last.
