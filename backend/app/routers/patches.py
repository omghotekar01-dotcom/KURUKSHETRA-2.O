...
def _approve_patch(incident_id: str, payload: PatchDecisionRequest, idempotency_key: str) -> PatchExecutionResult:
    ...
    if previous is not None:
        store.append_event(incident_id, "PATCH_EXECUTION_REUSED", "Duplicate approval safely returned the existing remediation result.", {"proposal_id": payload.proposal.proposal_id, "idempotency_key": idempotency_key, "branch": previous.branch, "commit_sha": previous.commit_sha, "draft_pr_number": previous.draft_pr_number, "draft_pr_url": previous.draft_pr_url})
        store.set_status(incident_id, IncidentStatus.verifying)
        return previous
    try:
        context = collect_repository_context(incident)
        fresh = build_patch_proposal(incident, context, PatchProposalRequest(commit_sha=payload.proposal.base_commit, filename=payload.proposal.file_path, hunk_header=payload.proposal.hunk_header))
    except (GitHubContextUnavailable, PatchProposalUnavailable) as exc:
        store.append_event(incident_id, "PATCH_STALE", "Patch approval blocked because live evidence could not reproduce the reviewed proposal.", {"proposal_id": payload.proposal.proposal_id, "reason": str(exc), "idempotency_key": idempotency_key})
    ...
    if not _same_exact_proposal(fresh, payload.proposal):
        store.append_event(incident_id, "PATCH_STALE", "Patch approval blocked because the live proposal changed since review.", {"proposal_id": payload.proposal.proposal_id, "fresh_proposal_id": fresh.proposal_id, "idempotency_key": idempotency_key})
    ...
    store.append_event(incident_id, "PATCH_APPROVED", f"Exact patch proposal {fresh.proposal_id} approved by {payload.reviewer}.", {"proposal_id": fresh.proposal_id, "reviewer": payload.reviewer, "note": payload.note, "repository": fresh.repository, "file_path": fresh.file_path, "base_commit": fresh.base_commit, "idempotency_key": idempotency_key})
    store.set_status(incident_id, IncidentStatus.executing)
    result = execute_approved_patch(incident, fresh)
    store.append_event(incident_id, "PATCH_EXECUTION", result.message, {"proposal_id": result.proposal_id, "status": result.status, "repository": result.repository, "branch": result.branch, "branch_url": result.branch_url, "commit_sha": result.commit_sha, "draft_pr_number": result.draft_pr_number, "draft_pr_url": result.draft_pr_url, "validation": [check.model_dump() for check in result.validation], "reused": result.reused, "idempotency_key": result.idempotency_key or idempotency_key})
...
def decide_patch(incident_id: str, payload: PatchDecisionRequest) -> PatchExecutionResult:
    ...
    if payload.decision is ApprovalDecision.reject:
        store.append_event(incident_id, "PATCH_REJECTED", f"Patch proposal {payload.proposal.proposal_id} rejected by {payload.reviewer}.", {"proposal_id": payload.proposal.proposal_id, "reviewer": payload.reviewer, "note": payload.note})
        refreshed = store.get(incident_id)
        status = refreshed.status if refreshed else incident.status
        return PatchExecutionResult(incident_id=incident_id, proposal_id=payload.proposal.proposal_id, decision=payload.decision, status="REJECTED", message="Patch proposal rejected. No repository write occurred.", repository=payload.proposal.repository, incident_status=status)
...
def patch_verification(incident_id: str) -> PatchVerificationResult:
    ...
    if not repository or not commit_sha or not draft_pr_number:
        raise HTTPException(status_code=409, detail="No executed remediation with a draft PR is available for CI verification.")
    try:
        result = verify_patch_ci(incident, repository=repository, commit_sha=str(commit_sha), draft_pr_number=int(draft_pr_number))
    ...
    if changed:
        store.append_event(incident_id, "PATCH_CI_VERIFICATION", result.message, {"status": result.status, "repository": result.repository, "commit_sha": result.commit_sha, "draft_pr_number": result.draft_pr_number, "draft_pr_url": result.draft_pr_url, "pr_state": result.pr_state, "pr_draft": result.pr_draft, "checks": [check.model_dump() for check in result.checks], "derived_incident_outcome": result.derived_incident_outcome.value if result.derived_incident_outcome else None, "verification_evidence": result.verification_evidence, "runtime_verification_required": result.runtime_verification_required})
        store.append_event(incident_id, "VERIFICATION_DERIVED", "Incident verification evidence was derived from the real remediation commit and GitHub CI state.", {"source": "github-ci", "ci_status": result.status, "derived_outcome": result.derived_incident_outcome.value if result.derived_incident_outcome else None, "evidence": result.verification_evidence, "runtime_verification_required": result.runtime_verification_required})
    store.set_status(incident_id, result.incident_status)
    return result
